...
void process_clause_removal(HeuristicsDB &db, const std::set<long long>& clause) {
    for (auto &c_lit: clause)
        --db[c_lit];
}

/**
 * sterge literalul pur
 * @param cs
 * @param single_literals
 * @param result
 * @param db
 * @return
 */
...
SatState davis_putnam_logemann_loveland(const HeuristicsDB &db, const ClauseSet &cs) {
    SatState result = SatState::NECUNOSCUT;
    ...
    while (!context_stack.empty()) {
        ...
        c_context.get_db().populate_literal_record();
        ...
        if (!c_solved){
            ...
            literal_ctx.get_db().populate_literal_record();

            negative_literal_ctx.get_cs().emplace(notLiteral);
            ++negative_literal_ctx.get_db()[-l];
            negative_literal_ctx.get_db().populate_literal_record();
...
int main(int argc, const char* argv[]) {
    ...
    if (argc != 3) {
        std::cerr<<"Wrong input. Usage .\\dpll_rand.exe <path_to_cnf_file> <path_to_log_file>";
        return 1;
    }
    if (!fs::exists(argv[1])) {
    ...
    std::ofstream g(argv[2],std::ios::app);
    int poz;
